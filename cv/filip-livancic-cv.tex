...
\begin{tabular}{@{} l p{16cm} @{}}
  ...
  \textbf{Data Engineering} & SQL, Python, Tableau, dbt, Git, Docker, Terraform, Jupyter, Linux(Arch, Debian), Shell(bash, zsh), Dash \\
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep]
  \item Built an end-to-end analytics pipeline with monthly API ingestion into GCS, data modeling in BigQuery, and interactive visualisations in Plotly Dash hosted on Cloud Run.
  \item Leveraged Pub/Sub triggers to launch Compute Engine (VM) instances via Cloud Run, with automatic VM teardown upon workload completion.
  \item Integrated GitHub Actions and Terraform to automate infrastructure updates on pull request merges.
\end{itemize}
